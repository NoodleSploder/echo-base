...
// on the same poll cadence as ReceiversPanelWidget. The waterfall
// itself stays a decorative sample animation (see MiniWaterfall) since
// there's no IQ streaming transport yet -- see ROADMAP.md.
    ...
    <Panel title="Receiver Tiles" bodyClassName="p-3">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {receivers.map((receiver, index) => {
          const status = statuses[receiver.id];
          ...
          return (
            <div key={receiver.id} className="rounded-md border border-base-700 bg-base-800/40 p-2">
              ...
              <div className="mb-1 font-mono text-lg text-slate-100">
                {formatFrequency(status?.frequency_hz ?? null)}
              </div>
